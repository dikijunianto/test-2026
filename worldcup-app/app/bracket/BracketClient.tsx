...
export function BracketClient({ teams, groups, standings }: BracketClientProps) {
  ...
  const [bracketState, setBracketState] = useState<BracketState>(() =>
    generateBracket(qualification)
  );

  const handleSelectWinner = useCallback(
    (matchId: string, winnerId: number) => {
      setBracketState((prev) => selectWinner(prev, matchId, winnerId));
    },
    []
  );

  const getRoundName = (round: BracketMatch["round"]): string => {
    switch (round) {
      case "roundOf32":
        return "Round of 32";
      case "roundOf16":
  ...
  const renderMatch = (match: BracketMatch) => {
    ...
    return (
      <div
        key={match.id}
        className="bg-white border rounded-lg shadow-sm p-3 min-w-[200px]"
      >
        <button
          onClick={() =>
            match.homeTeamId && handleSelectWinner(match.id, match.homeTeamId)
          }
        ...
        <button
          onClick={() =>
            match.awayTeamId && handleSelectWinner(match.id, match.awayTeamId)
          }
    ...
    <div className="space-y-6">
      {bracketState.champion && (
        <div className="bg-yellow-50 border-2 border-yellow-400 rounded-lg p-6 text-center">
          ...
          <div className="flex items-center justify-center gap-3">
            {teamMap.get(bracketState.champion)?.crest && (
              <img
                src={teamMap.get(bracketState.champion)!.crest}
                alt=""
                className="w-12 h-12"
              />
            )}
      ...
      <div className="bg-white rounded-lg shadow p-6 overflow-x-auto">
        <div className="flex gap-8 min-w-max">
          {rounds.map((round) => {
            const matches = getMatchesForRound(bracketState, round);
            return (
